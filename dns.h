...
typedef struct dns_raw_resource_record{
	u_int16_t length;
	char *data;
} dns_raw_resource_record_t;

typedef struct dns_address_record{
	struct in_addr addr;
} dns_address_record_t;

typedef struct dns_in6_address_record{
	struct in6_addr addr;
} dns_in6_address_record_t;

typedef struct dns_domain_name_record{
	char *name;
...
typedef struct dns_HINFO_record{
	char *cpu;
	char *os;
} dns_HINFO_record_t;

typedef struct dns_MINFO_record{
	char *rmailbx;
	char *emailbx;
} dns_MINFO_record_t;

typedef struct dns_MX_record{
...
typedef struct dns_RP_record{
	char *mailbox;
	char *txtdname;
} dns_RP_record_t;

typedef struct dns_AFSDB_record{
	u_int32_t subtype;
	char *hostname;
} dns_AFSDB_record_t;

typedef struct dns_X25_record{
	char *psdn_address;
} dns_X25_record_t;

typedef struct dns_ISDN_record{
	char *isdn_address;
	char *subaddress;
} dns_ISDN_record_t;

typedef struct dns_RT_record_t{
	u_int16_t preference;
	char * intermediate;
} dns_RT_record_t;

typedef struct dns_LOC_record{
	u_int8_t version;
	u_int8_t size;
